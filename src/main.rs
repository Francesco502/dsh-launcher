...
use windows_sys::Win32::Foundation::{
    ...
    ERROR_NO_MORE_FILES, FILETIME, HANDLE, HANDLE_FLAG_INHERIT, HWND, INVALID_HANDLE_VALUE,
...
fn instance_identity(executable: &Path) -> String {
    format!("DeepSeek.DSHLauncher.{:016x}", instance_hash(executable))
}

fn acquire_single_instance() -> Option<MutexGuard> {
    let executable = env::current_exe().ok()?;
    let name = format!("Local\\{}", instance_identity(&executable));
...
fn show_error_box(_: HWND, text: &str) {
    // Only startup failures before the Slint event loop use the OS fallback.
...
mod tests {
    ...
    fn reopen_window_identity_matches_the_installation_mutex() {
        ...
        assert_eq!(instance_identity(first), instance_identity(same));
        assert_ne!(instance_identity(first), instance_identity(other));
        assert!(instance_identity(first).ends_with(&format!("{:016x}", instance_hash(first))));
    }
