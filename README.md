...
点击底部版本号打开“关于”，其中的“检查更新”检查正式 GitHub Release；确认新版后下载并验证便携 ZIP、SHA-256 与发布清单，以事务替换四个程序文件并重新打开窗口。新版未就绪时恢复整组旧文件。已是最新版、取消或检查失败均保留窗口。0.3.3 首次升级仍需完整安装 0.4.0，一键更新支持后续版本。退出启动器保留 DSH；停止优先请求 DSH 释放资源，12 秒内未退出则清理已核实的进程树。托盘固定为“打开面板”“启动/停止 DSH”“打开 Web UI”“退出”。右上角关闭会释放面板与绘制资源，托盘继续运行；托盘右键“退出”才完全退出启动器，DSH 服务和日志助手继续运行，重新打开面板会检测已有服务。悬停只显示“DSH运行中”或“DSH未启动”；正常启动、停止和隐藏窗口均不弹系统通知。隐藏后停止 15 秒健康检查，不会周期运行 `netstat.exe`。
...
0.5.1 起，“更新所选插件”更新当前 profile **已保存的启用插件**；未保存的勾选必须先保存或取消。按原 npm 源选择包含预发布的最高 SemVer，不降级，确认后写入精确版本与锁文件。未选直接依赖保留锁定版本，本地路径和 Git 来源不自动更新，结果中列出“需在原来源更新”。缺失来源、锁文件不完整或配置变化时停止操作，不猜测公共源。更新插件不执行包安装脚本。

“更新 DSH”同时检查这些插件；DSH 已是最新版时仍可单独更新插件。候选完整验证后统一提交，失败恢复原组合；运行中的 DSH 在提交前短暂停止，成功或回滚后恢复运行，原本停止则保持停止。
...
启动器校验包名、精确版本、`lib/bin.js`、直接运行依赖、原生模块实际加载、内置持久化组件导入、Node 要求、boot 接口和配置/插件入口后，交换完整的管理 npm 前缀。0.5.1 联合更新保留原组合至提交及启动验证完成；失败或提交中断会恢复原组合。旧版遗留安装事务仍按其原有协议恢复，新事务不会混用旧版的继续提交策略。
