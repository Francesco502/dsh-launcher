...
点击底部版本号打开“关于”，其中的“检查更新”检查正式 GitHub Release；确认新版后下载并验证便携 ZIP、SHA-256 与发布清单，以事务替换四个程序文件并重新打开窗口。新版未就绪时恢复整组旧文件。已是最新版、取消或检查失败均保留窗口。0.3.3 首次升级仍需完整安装 0.4.0，一键更新支持后续版本。退出启动器保留 DSH；停止优先请求 DSH 释放资源，12 秒内未退出则清理已核实的进程树。托盘提供“打开面板”、与主窗口同步的主操作、“打开 Web UI”和“退出”。右上角关闭会释放面板与绘制资源，托盘继续运行；托盘右键“退出”才完全退出启动器，DSH 服务和日志助手继续运行，重新打开面板会检测已有服务。悬停只显示“DSH运行中”或“DSH未启动”；正常启动、停止和隐藏窗口均不弹系统通知。隐藏后停止 15 秒健康检查，不会周期运行 `netstat.exe`。
...
“更新已启用插件”更新当前 profile **已保存的启用插件**；未保存的勾选必须先保存或取消。按原 npm 源选择包含预发布的最高 SemVer，不降级，确认后写入精确版本与锁文件。未选直接依赖保留锁定版本，本地路径和 Git 来源不自动更新，结果中列出“需在原来源更新”。缺失来源、锁文件不完整或配置变化时停止操作，不猜测公共源。更新插件不执行包安装脚本。

0.5.2 起，“更新 DSH”和 CLI `upgrade` 仅更新 DSH 及其自带组件，不查询或升级第三方插件，不修改插件依赖、锁文件和启用设置。DSH 已是最新版且无需修复时直接结束。候选仍对现有插件进行本地预检；预检失败保留原安装，提交后启动失败恢复原版本。运行中的 DSH 在提交前短暂停止，成功或回滚后恢复运行，原本停止则保持停止。第三方插件通过插件页的独立入口更新。

结果区分提交前失败、失败后已恢复和恢复未完成；错误摘要始终显示恢复结果及失败原因，完整诊断可展开查看。已成功更新但旧文件清理失败时，新版保留，正常启动和停止仍可使用；事务记录保留供重试，下次更新必须先完成旧清理。

托盘主操作与面板共用动作名称和可用状态，包括安装、启停及可取消任务；弹窗打开时禁用会被拦截的菜单操作。放弃未保存的插件勾选时，可选择“继续编辑”或“放弃修改”；已保存设置保持不变。
...
启动器校验包名、精确版本、`lib/bin.js`、直接运行依赖、原生模块实际加载、内置持久化组件导入、Node 要求、boot 接口和配置/插件入口后，交换完整的管理 npm 前缀。更新保留原版本至提交及启动验证完成；失败或提交中断会恢复原版本。0.5.1 遗留联合事务继续整体恢复，旧版安装事务仍按原协议恢复。

对于要求 Web 认证的 DSH 版本，启动器读取并重新验证 DSH 自己输出的本地认证地址用于健康检查和“打开 Web UI”，包括 DSH 0.1.7 返回的相对路径认证跳转；但状态栏、托盘和通知不会显示认证令牌。普通占用 3080 端口的 HTTP 程序不会被识别为 DSH。
...
认证日志采用增量读取，日志增长或中文字符不会使有效地址丢失。认证失效时，界面说明停止后重新启动的恢复方法。旧版遗留安装事务候选缺失或损坏时进入重装流程，保留配置模式，不会把空目录当作恢复成功。
...
项目自身代码使用 [MIT](LICENSE)。界面使用 [Slint](https://slint.dev)，按 [Slint Royalty-free License 2.0](licenses/Slint-Royalty-free-2.0.md) 使用；0.5.1 起采用公开网页署名方式。Slint 及其依赖保持各自许可，发布资产中的 SPDX SBOM 列出 Rust 依赖版本。本项目的 MIT 许可不覆盖或重新授权这些第三方代码。
