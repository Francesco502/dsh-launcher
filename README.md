...
`DSH-Launcher.exe` 是 Windows 10/11 x64 的轻量原生 DSH 启动器。0.3.3 提供启动/停止、重启、打开 Web UI、手动更新 DSH 和逐个插件选择；普通启动只检查本机，不联网。右下角系统通知仅用于报错。
...
- “选择插件”列出已安装的第三方插件；停止 DSH 后可保存勾选结果，下次启动生效。不兼容的插件需要保持取消勾选，选择启用不会修复插件缺失的接口。

页脚“检查启动器更新”只比较 GitHub Release 版本；发现新版后打开官方 Release 页面，不下载或替换当前 EXE。托盘固定为“打开面板”“启动/停止 DSH”“打开 Web UI”“退出”。窗口关闭时立即隐藏；正常启动、停止和隐藏窗口均不弹系统通知。隐藏后停止 15 秒健康检查，不会周期运行 `netstat.exe`。
