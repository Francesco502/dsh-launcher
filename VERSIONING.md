...
- `0.3.3` 按用户明确指定，作为一次限定范围的 0.x 版本例外，加入逐个插件选择、窗口重启按钮并收缩系统通知；不复用已公开的 `0.3.2`，后续版本继续遵循通常规则。
...
0.5.0 使用固定版本 Slint 1.17.1、winit 与软件渲染；EXE 体积预算改为 16 MiB。此预算不代表内存占用。发布前另行记录可见/托盘私有工作集、Private Bytes、60 秒 CPU、真实首次交互、重开及反馈延迟，执行 100 次窗口生命周期检查，并覆盖当前远程会话实际 100% DPI。用户于 2026-09-09 明确取消 150% DPI 及本地显示验收，并允许真实按钮反馈 P95 ≤100 ms 标为未验证后发布；该数值不得标为通过。未完成的显示项目必须标记未验收，不能用构建成功或离屏绘制代替。下载页须附 Slint 署名和固定版本许可链接，保留六个发布资产及四个便携程序文件。
...
10. 下载正式便携 ZIP 并按 `.sha256` 校验；0.4.0 起应用内确认更新后自动验证、事务替换四个程序文件，并通过新版就绪握手验证启动。首次从 0.3.3 升级须完整安装便携包。
